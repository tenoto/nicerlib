...
class ExposureMap_IBGvsHREJ():
	...
	"""
	def __init__(self):
		self.matrix_ibg52_hrej52 = np.zeros((NUM_Ofname_HREJ,NUM_Ofname_IBG))
...
class ExposureMap_NZ():
	...
	"""
	def __init__(self):
		self.hist_nz52 = np.zeros(NUM_Ofname_NZ)
...
num_ofname_fpm = 56 - 4 - len(exclude_detid_list) 
...
""" %  (fname_ufatsel_screen,tstart,tstop,fname_ufatsel_screen_gti_nz_lc,subdir)
	...
	dump = '%s %d %.4f %.4f %.4f' % (fname_clevt_screen_gti_pha,num_ofname_fpm,
		gti_ibg_average,gti_hrej_average,gti_nz_average)
	fin.write(dump+'\n')

	dump  = '%.1f %.1f %d %s %.1f ' % (tstart,tstop,gtinum,obsid,texp)
	dump += '%d %.4f %.4f %.4f ' % (num_ofname_fpm,gti_ibg_average,gti_hrej_average,gti_nz_average)
	dump += '%.4f %.4f %.4f ' % (
		52.0 / float(num_ofname_fpm) * gti_ibg_average,
		52.0 / float(num_ofname_fpm) * gti_hrej_average,
		52.0 / float(num_ofname_fpm) * gti_nz_average
		)
	flog.write(dump+'\n')
fin.close()
flog.close()
...
# ==============================
expr_list = []
for i in range(NUM_Ofname_HREJ):
	for j in range(NUM_Ofname_IBG):
...
for k in range(1,NUM_Ofname_NZ-1):
...
cmd  = 'mathpha '
cmd += 'expr=@%s ' % fname_mathpha_expr
cmd += 'units=R outfil="%s" ' % fname_bgd_pha
cmd += 'exposure=1.0 ' 
#cmd += 'exposure=%s ' % fname_clpha_screen_gti
